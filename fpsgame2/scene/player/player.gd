...
func process_movement_inputs():
	...
	# normalizing the input diredctions(what does this mean?????)
	dir = dir.normalized()
...
func process_movement(delta):
	#set speed anmd target velocity
	var speed = SPRINT_SPEED if Input.is_action_pressed("sprint") else SPEED
	var target_vel = dir * speed 
	
	#smooth out the players movement
...
func _input(event):
	if event is InputEventMouseMotion:
		# Rotates the veiw vertically
		$CamRoot.rotate_x(deg2rad(event.relative.y * MOUSE_SENSITIVITY * -1))
		$CamRoot.rotation_degrees.x = clamp($CamRoot.rotation_degrees.x, -75, 75)#<------ remove line to do backflip!!!!!
		# rotates the mouse horizontally
